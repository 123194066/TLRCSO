function [Best_Score, Convergence_curve, BestFit, f1_zong, f2_zong, f3_zong, f4_zong, BestIndex] = TLRCSO(SearchAgents_no, dim, Max_iteration, ub, lb, pos_tag, all_param)
...
for t = 1:Tma
    ...
    disp(['TLRCSO iteration = ', num2str(t), '   ', 'BEST Fitness = ', num2str(fMin)]);
 ...
 mother= randi(rNum, cNum, 1);  % 公鸡索引
    ...
    for i = 1:rNum
        NW = NWmax - (NWmax - NWmin) * t / Tma;
          cut =0.7*exp(-0.005*t+0.3);
        
        pd_truncated = truncate(pd_normal, -cut, cut); %%%%%%%% 截断高斯对象
       ...
       zero_contribution_idx = find(temp_alpha_m == 0);
        ...
        f1_buf = zeros(1, 5); f2_buf = zeros(1, 5); f3_buf = zeros(1, 5); f4_buf = zeros(1, 5);
          ...
          for iii = 1:5
            ...
            pX(sortIndex(i), (r_index-1)*2+1:r_index*2) .* (1 + tempSigma .* random(pd_truncated, 2, 1)' );
...
beta = 1.0 + (t / Tma) * 0.8;  
    ...
    update_count = randi([2,5]);  
    random_index = randi([1, dim/2], [1, update_count]);  
    hen_update_index = zeros(1, dim);
    hen_update_index([random_index.*2-1; random_index.*2]) = 1;  

    if rand < 0.08
        random_disturb_index = randi(dim);  
